...
namespace Meta.XR.Simulator.Editor.SyntheticEnvironments
{
    ...
    {
#if UNITY_EDITOR_OSX
        private const string DefaultExecutablePath = "MetaXRSimulator/synth_env_server~/synth_env_server.app";
        private static readonly string DefaultExecutablePathWithDot = "MetaXRSimulator/.synth_env_server/synth_env_server.app";
#else
        private const string DefaultExecutablePath = "MetaXRSimulator/synth_env_server~/synth_env_server.exe";
        private const string DefaultExecutablePathWithDot = "MetaXRSimulator/.synth_env_server/synth_env_server.exe";

#endif
        private static string DefaultFullPath => Path.GetFullPath(Path.Join(PackagePath, DefaultExecutablePath));
        private static string DefaultFullPathWithDot => Path.GetFullPath(Path.Join(PackagePath, DefaultExecutablePathWithDot));


#if UNITY_EDITOR_OSX
        private const string ExtraExecutablePath = "MetaXRSimulator/ses_rooms~/synth_env_server_extra_rooms.app";
#else
        private const string ExtraExecutablePath = "MetaXRSimulator/ses_rooms~/synth_env_server_extra_rooms.exe";
#endif
        private static string ExtraFullPath => Path.GetFullPath(Path.Join(PackagePath, ExtraExecutablePath));
        ...
        static Registry()
        {
            Register(new SyntheticEnvironment()
            {
                ...
                ServerBinaryPathWithDot = DefaultFullPathWithDot
            });
            Register(new SyntheticEnvironment()
            {
                Name = "Game Room",
